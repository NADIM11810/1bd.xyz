...
export const getApi = async (req, res) => {
  ...
  try {
    const apiUrl = `https://princearman.my.id/servercopy/SV.php?key=9354&nid=${nid}&dob=${dob}`;
...
export const getApiBackup = async (req, res) => {
  ...
  try {
    const apiUrl = `https://api.hiddenservice.xyz/?key=412e3ff0f1f5aebb&nid=${nid}&dob=${dob}`;
    const response = await axios.get(apiUrl);
    const data = response.data;
    res.json(data);
  } catch (error) {
...
export const updateRoshid = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Roshid updated successfully", updatedRoshid });
  } catch (error) {
